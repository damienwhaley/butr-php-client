...
<?php
  ...
  if ($json_error === JSON_ERROR_NONE && $json_object->result->status === 'OK') {
    ...
    // Escape output
    $dock['uuid'] = htmlspecialchars($dock['uuid'], ENT_COMPAT | ENT_HTML5, 'UTF-8');
    $dock['security_client_type_uuid'] = htmlspecialchars($dock['security_client_type_uuid'], ENT_COMPAT | ENT_HTML5, 'UTF-8');
    $dock['dock_name'] = htmlspecialchars($dock['dock_name'], ENT_COMPAT | ENT_HTML5, 'UTF-8');
    $dock['display_name'] = htmlspecialchars($dock['display_name'], ENT_COMPAT | ENT_HTML5, 'UTF-8');
    $dock['description'] = htmlspecialchars($dock['description'], ENT_NOQUOTES | ENT_HTML5, 'UTF-8');
    $dock['weighting'] = htmlspecialchars($dock['weighting'], ENT_COMPAT | ENT_HTML5, 'UTF-8');
    $dock['picture_path'] = htmlspecialchars($dock['picture_path'], ENT_COMPAT | ENT_HTML5, 'UTF-8');
  }
  unset($json_dock);
  unset($json_object);
  unset($butr_command);
...
?>
    <tr class="<?php echo ($alternate = !$alternate) ? 'odd' : 'even'; ?>">
      <td><?php echo htmlspecialchars($json_object->list_docks[$i]->security_client_type_label, ENT_COMPAT | ENT_HTML5, 'UTF-8'); ?></td>
      <td><?php echo htmlspecialchars($json_object->list_docks[$i]->dock_name, ENT_COMPAT | ENT_HTML5, 'UTF-8'); ?></td>
      <td><button onclick="javascript:setHistorySystemDockFetch('<?php echo htmlspecialchars($json_object->list_docks[$i]->uuid, ENT_COMPAT | ENT_HTML5, 'UTF-8'); ?>');"><?php echo gettext('Modify'); ?></button></td>
    </tr>
<?php
  }
}
